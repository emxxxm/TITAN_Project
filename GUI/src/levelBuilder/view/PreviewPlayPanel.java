...
public class PreviewPlayPanel extends JPanel{
	...
	public int determineMulti(double freq){
		...
		else if(bonusFreq.getx2()<freq && freq<bonusFreq.getx2()+bonusFreq.getx3()){
			num=3;
		}
	...
	protected void paintComponent(Graphics graph)
	{
		...
		ArrayList<Position> disabled = model.getDisableSquareList().getDisableSquareList();
		...
		for(int row=0;row<9;row++)
		{
			for(int col=0;col<9;col++)
			{
				...
				if(squares[row*9+col]==1)
				{
					...
					graph.setColor(Color.white);
					graph.drawString(Integer.toString(squareNum), (int)(57+col*47+57+col*47+40)/2, (int)(7+row*47+7+row*47+40)/2);
					int squareMulti=determineMulti(freq);
					if(squareMulti!=0){
						graph.drawString("X"+(Integer.toString(squareMulti)),(int)(57+col*47+57+col*47+40)/2+5, (int)(7+row*47+7+row*47+40)/2+18);
					}
				}
			}
